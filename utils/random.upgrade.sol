...
contract randomupgrade is OwnableUpgradeable {
    uint256 _seed;
    uint256 _b;
    uint256 _c;
    uint256 _m;
    ...
    constructor() {
        //_disableInitializers(); //开发阶段不要用ERC1967
    }
    function initialize(uint256 seed,uint level,uint b,uint c,uint m) public initializer  {
        __Ownable_init();
        uint256 pre;
        _level=level;
        pre=seed;
    ...
    function setseed(uint256 seed) public onlyOwner virtual {
        _setseed(seed);
    }
